...
RETURN FORMAT: Return a JSON object with these fields:
{
  ...
  "imageBrief": "A specific visual brief matching the assigned visual type for this post. For DATA CARDS: specify the exact stat, subtext, and source to display. For PADDOCK PHOTOS: suggest which type of paddock photo would reinforce this post (grid walk, debrief, pit lane, etc). For AI IMAGES: describe the neuroscience visualisation concept. For TEXT QUOTE CARDS: write the exact quote to display. For CAROUSEL DOCUMENTS: outline all 6-8 slide headlines.",
